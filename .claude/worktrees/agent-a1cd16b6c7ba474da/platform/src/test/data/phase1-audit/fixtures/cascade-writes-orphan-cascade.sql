-- MUST FAIL: tests code rejects cascade edge_history row that references a non-existent reasoning_report_id — FK violation on reasoning_report_id.
-- cascade-writes-orphan-cascade.sql
-- Phase 1 adversarial variant — dangling FK on cascade row
-- Complexity score: 22 (rows=7, edges=1, stressors=1, adversarial-weighted)
-- Stressors: adversarial-structural=dangling-fk
--
-- UUID map: reuses cascade-writes seed.
-- Adversarial delta:
--   60000000-0000-0000-0000-0000000000ff  orphan cascade row (target of FAIL)
--   50000000-0000-0000-0000-0000deadbeef  phantom report (never inserted)

BEGIN;

-- Minimal seed (single upstream fact + edge + report) -----------------------
INSERT INTO public.entities (id, canonical_name, entity_type, embedding, confidence) VALUES
  ('00000000-0000-0000-0000-000000000001', 'Alpha Corp', 'company', ARRAY(SELECT random() FROM generate_series(1, 768))::vector, 1.0),
  ('00000000-0000-0000-0000-000000000002', 'ProductX',   'product', ARRAY(SELECT random() FROM generate_series(1, 768))::vector, 1.0)
ON CONFLICT (id) DO NOTHING;

INSERT INTO public.facts (id, subject_entity_id, predicate, object_entity_id, confidence, valid_at) VALUES
  ('10000000-0000-0000-0000-000000000001',
   '00000000-0000-0000-0000-000000000001', 'manufactures',
   '00000000-0000-0000-0000-000000000002', 0.9, NOW())
ON CONFLICT (id) DO NOTHING;

INSERT INTO public.reasoning_reports (id, mode, report, created_at) VALUES
  ('50000000-0000-0000-0000-000000000001', 'patrol', 'Seed report', NOW())
ON CONFLICT (id) DO NOTHING;

INSERT INTO public.causal_events (id, fact_id, transition_type, source_text, occurred_at) VALUES
  ('20000000-0000-0000-0000-000000000001', '10000000-0000-0000-0000-000000000001', 'created',
   'Upstream fact observation', NOW())
ON CONFLICT (id) DO NOTHING;

INSERT INTO public.causal_edges
  (id, cause_event_id, effect_event_id, strength, reasoning, source_references,
   extraction_method, initial_strength, created_at) VALUES
  ('30000000-0000-0000-0000-000000000001',
   '20000000-0000-0000-0000-000000000001',
   '20000000-0000-0000-0000-000000000001',
   0.80, 'Seed edge for adversarial test',
   '[{"type":"fact","id":"10000000-0000-0000-0000-000000000001","relevance":"primary"}]'::jsonb,
   'fixture', 0.80, NOW())
ON CONFLICT (id) DO NOTHING;

-- ADVERSARIAL DELTA: cascade row pointing at nonexistent report -------------
-- STRESSOR: adversarial-structural=dangling-fk
INSERT INTO public.causal_edge_history
  (id, edge_id, event_type, previous_strength, new_strength,
   reasoning, reasoning_report_id, actor, occurred_at) VALUES
  ('60000000-0000-0000-0000-0000000000ff',
   '30000000-0000-0000-0000-000000000001',
   'weakened', 0.80, 0.60,
   'Orphan cascade row — parent report does not exist',
   '50000000-0000-0000-0000-0000deadbeef', -- phantom
   'cascade',
   NOW());

COMMIT;
