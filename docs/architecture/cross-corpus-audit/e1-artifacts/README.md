...
## Leg 7 — F.2 with HUMAN ground truth (the settling experiment) → pilot INCONCLUSIVE (doc §25–§26)
The missing cell: a genuinely no-structural-shadow rule (F.2 "does one thing") with a HUMAN oracle, at field prevalence. Pilot n=30 (seeded random). Human 9/30 violations; three LLM runs — Opus orig / Haiku / Opus on the human's verbatim sheet — ALL 1/30, all flagging only `doCoarsePositioning`; κ human-vs-LLM 0.15, κ LLM-vs-LLM 1.00. Intended conclusion "LLM can't audit F.2, close" was **CUT** by adversary: sole human oracle flagged an intentionally-empty `reset()` as a violation (near-objective error → oracle not credible); κ CI [0.00, 0.47] at n=30; κ=1.0 is an artifact (2 runs same model ≈1 effective labeller); all LLMs caught the one blatant case with zero clear false-negatives (diverge only in the ambiguous mid-zone); over-generalized from the most subjective rule. **Second human labeller is the load-bearing, non-optional test.** §24 disposition must not cite this leg.
- `build_leg7.mjs` (sample + blind sheet), `leg7_sample.json` (300-fn seeded order), `leg7_pilot_sheet.md` (human labels, filled)
- `leg7_subagent_labels.json` (Opus orig), `leg7_haiku_labels.json`, `leg7_opus_verbatim_labels.json` (three blind LLM runs on the same 30)
