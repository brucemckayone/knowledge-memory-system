...
const key = JSON.parse(readFileSync(`${OUT}/leg2_key.json`, 'utf8'));
const resp = JSON.parse(readFileSync(`${OUT}/leg2_responses.json`, 'utf8'));
...
import('node:fs').then(fs=>fs.writeFileSync(`${OUT}/leg2_disagreements.json`, JSON.stringify(disagreements,null,2)));
