...
1. **Graph C was empty for a one-line reason.** The identical 294 documents produced **9** causal edges
   before the fix and **975** after — same denominator, ~108x per document, all with reasoning. **[M]**
...
| 2026-08-31 (post-fix, this session) | **975** | 294 | **3.3** |

**Same denominator, so this is apples to apples:** the identical 294 documents produced 9 causal edges
before the fix and **975 after** — a ~108x increase in edges per document. All 975 carry non-blank
`reasoning`. (An earlier version of this table quoted 120 edges from 30 documents, a mid-ingest snapshot;
the completed run supersedes it.)
