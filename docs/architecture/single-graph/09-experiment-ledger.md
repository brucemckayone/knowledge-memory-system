...
- **Retrieval-track consecutive ties:** **1** (pool-then-re-rank). Two more ties ⇒ stop the retrieval
  track.
- **Oracle status:** condensed oracle (E0) characterised and adopted; not the binding constraint.
- **Open bug-hunt leads:** background sweep in flight (see BH-1). Substrate: 70.6% of query targets have a
  same-name duplicate entity (dedup off) — quantified in E0, low retrieval impact (removing dups recovers
  1 pair).
...
| R1 | retrieval | `08` | `pool-rerank-results.json` | B(P=100 DESC-pool→NAME-rerank) − ARM-NAME = **−0.0056 [−0.0169,+0.0056]**, spans 0 — **TIE**. B≈NAME (0.1949 vs 0.2006); control B′ worse (−0.0508); B beats DESC (+0.0565) | pending | ⏳ `553e111` (pre-adversary) |
| BH-1 | bug hunt | — | systematic silent-no-op sweep | — | in flight | — |
