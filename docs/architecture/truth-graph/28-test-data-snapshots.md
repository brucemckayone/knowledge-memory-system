...
- **`pg_dump` location**: Postgres client tools may not be on PATH. Scripts probe via Node's `child_process` rather than shell builtins (cold-eyes review S11): `process.platform === 'win32' ? execSync('where pg_dump') : execSync('which pg_dump')` — but `where` is a CMD builtin and unreliable from Git Bash. Preferred: `process.env.PG_DUMP_PATH` if set; otherwise `child_process.spawnSync('pg_dump', ['--version'])` to test if it's resolvable; fall back to common install paths (`C:\Program Files\PostgreSQL\*\bin\pg_dump.exe` on Windows; `/usr/bin/pg_dump`, `/usr/local/bin/pg_dump` on Unix)
