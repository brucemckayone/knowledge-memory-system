...
**Tense, first — but not verb-lemmatization on the scored string.** Before scoring, `normalize_tense` folds tense aliases (`worked_at → works_at`) so tense becomes a `valid_at`/`invalid_at` timestamp (per the bi-temporal model), removing the false-merge cases that are really tense variants (B7's `works_at`/`worked_at`) before they reach the score. The temporal hint it emits **never overrides an explicit proposer-supplied `validAt`** (`valid_at` is the validity source of truth, per `41-epoch-v2-design.md` §12 #7) — it only fills the gap when the proposer left the fact undated. **Verb-lemmatization (`lemmatize_predicate`) is deliberately NOT applied to the string that is scored or guarded** (discovered during PC3 review): the inverse registry and the ConceptNet map key on *surface* forms (`employs`, `supervises`), and the enriched embedding already absorbs inflection (the validated benchmark scores surface forms directly). Lemmatizing `employs → employ` would silently defeat the inverse guard (it no longer matches the registered `employs`) and miss ConceptNet entries. `lemmatize_predicate` is still ported and unit-tested in `predicate_normalization` as an NL-preprocessing primitive. (`normalize_tense` is table-driven, not grammatical — see §6 and §12-R2.)
...
- Backfill **raw enriched** embeddings (uncentered) for the seeded canonicals. The merge decision scores on **uncentered enriched cosine** — that is the path the 2026-06-01 benchmark validated (b18/b19) and that the two thresholds were calibrated against (B3). Mean-centering is used only in B1's raw-vs-enriched *separation analysis*, not on the decision path, so there is no vocabulary-relative centering to keep consistent (this is the simpler resolution of §12-R1-N3 — confirmed against the benchmark in PC3). Query and candidate vectors are comparable because both use the identical enriched recipe.
...
3. **Enriched-embed** → vector (uncentered; the merge decision uses uncentered enriched cosine, §5). The service is **stateless** — the caller (PC4) loads the candidate canonicals from `fact_predicates` (with their stored PC2 embeddings) and passes them in; ml-services embeds only the query and scores.
...
"Tested and verified against real runs." The gate is **falsifiable**, not directional — concrete thresholds, not "up/down". It is run with **PC7 disabled** (deterministic-only configuration, so the determinism claims aren't contaminated by an async LLM) and at the **PC6-recommended operating point** (0.55/0.30/0.10/0.05 @ 0.89/0.84), which is *provisional until this gate's A/B validates it* against the old 0.905 point. Because the harness wipes and re-ingests fresh corpora per arm, the un-migrated live graph (§11) does not affect these numbers.

Re-run **both** the live validity harness and the ontology benchmark, versus the `cbf34d2` baseline (litmus fact-F1 0.27, determinism fact-F1 0.33, `predicateSprawlMax` 2). The criteria split into two tiers — this separation is the honest outcome of the PC8 review, which found that **absolute fact-F1 is floored by proposer over-extraction (filed `nmemo-m8d`), not by predicate canonicalization**:

**Tier A — epic-controllable (HARD gate; predicate canonicalization owns these):**
- **Hard-fail invariants (deterministic, ungameable):** duplicate entities = 0, duplicate facts = 0, `errorViolations` = 0, and **single-active-per-exclusive-group invariant pass-rate = 100%** (doc 39 §C) — the last directly proves canonicalization let group-supersession fire. (This subject-aware invariant is the authoritative "no sprawl" proof.)
- **`predicateSprawlMax` ≤ 1 (subject-aware)** on *both* corpora. NOTE: the sprawl metric is keyed on **(subject, exclusive-group)** — corrected during PC8 from a subject-blind version that wrongly counted a person's `lives_in` and an org's `headquartered_in` (different subjects, same coarse `location` group) as sprawl=2. Real sprawl = one subject's one relation split across predicate strings; that is what must be ≤1. (Driving the *subject-blind* count to ≤1 would require wrongly merging two distinct canonicals — a false-merge — which is why the metric, not the threshold, was the fix.)
- **No false-merge regression (A/B):** see the dedicated bullet below.
- **Ontology benchmark B7/B8/B9/B12 pass under multi-signal** (B8 per the bullet below).
- **Bounded vocabulary growth (explicit ceiling):** distinct active predicate count on corpus20 ≤ count(corpus10) + (genuinely-novel relations in chunks 11–20, enumerable from the gold graph), AND ≤ 1.5× the 27 seeded canonicals.
- **Fact litmus F1 and determinism fact-F1 UP vs the `cbf34d2` baseline** (0.27 / 0.33). Direction is the epic-controllable claim here.

**Tier B — extraction-gated (REPORTED, deferred to `nmemo-m8d`; NOT a hard gate for this epic):**
- **Absolute fact litmus F1 ≥ 0.70 / determinism ≥ 0.80** (with `litmus_F1 ≥ determinism_F1 − 0.05`). These aspirational levels are **blocked by proposer over-extraction precision** (fact precision 0.09–0.22 from non-gold triples), which is independent of predicate canonicalization. Report the numbers; do not gate the epic on them. They become gateable once `nmemo-m8d` lands.
- **No false-merge regression (measured, as an explicit A/B):** run the live harness at BOTH the old benchmark point (`PREDICATE_MERGE_THRESHOLD=0.905`, old weights) AND the PC6-locked point (0.89, new weights) via the env overrides, and require the locked point to (a) pass the absolute gate AND (b) not increase live false-merges vs the old point. A false-merge = a gold-distinct predicate pair collapsed to one canonical, measured against `gold/<corpus>.gold.json` (doc 39 §A); ontology-benchmark B7/B18 must also be 0. **Caveat the gate must check:** confirm corpus10/20 gold actually contains *same-type, same-category* distinct predicate pairs — if it doesn't, this live check inherits the curated-set blind spot (same-type pairs are where a too-low merge_t bites) and is not a real backstop; the PC6 sweep's augmented gate (same-type distinct-canonical pairs) is the compensating control. A merge is **lossy and unrecoverable** (the canonical string is written into `facts.predicate`); over-minting is gardener-recoverable — so on any doubt the threshold errs HIGH (the SAFETY_MARGIN in the sweep encodes this).
...
### PC8 result (2026-06-20, locked point 0.55/0.30/0.10/0.05 @ 0.89, PC7 off)

Tier A — all pass: subject-aware `predicateSprawlMax` = **0** (corpus10 both arms; the subject-blind metric was the bug, now fixed); dup entities/facts **0/0**; `errorViolations` **0**; `singleActivePerExclusiveGroup` **100%**; ontology benchmark **B7=0, B8=100%, B9=90.6%, B12=9/9**; vocabulary growth **36 ≤ 40.5** (corpus20); no false-merge regression (A/B 0.89 vs 0.905 — distinct canonicals kept separate at both); fact litmus F1 **0.27→0.311** (corpus10) / **0.552** (corpus20) and determinism fact-F1 **0.33→0.417** — both UP vs baseline. The fold fired strongly (reused 12–21, minted 0–8, deferred 0 per epoch).

Tier B — reported, deferred to `nmemo-m8d`: absolute fact-F1 stays well below the aspirational 0.70/0.80 because proposer over-extraction (precision 0.09–0.22 from non-gold triples) floors it — independent of predicate canonicalization.
...
- **N3 (synchronous embed / centering):** resolved in §5/§6 — store uncentered vectors and score on uncentered enriched cosine (the benchmark-validated path; no centering on the decision path, confirmed in PC3), so there is no vocabulary-relative centering to keep consistent; cache + batch; explicit ml-down deferral mode (PC4). The `/resolve-predicate` service is stateless (caller passes candidates), which also removes the synchronous-DB-in-promotion concern: the embed call is the only network hop and it is the query only.
