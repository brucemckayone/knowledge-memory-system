...
| 38 | [Parallel Ingestion](38-parallel-ingestion.md) | Parallelising agent extraction without reintroducing doc 05's temporal bugs (non-determinism, inverted temporal direction, cascade data loss). Two candidate architectures — epoch barrier vs continuous optimistic concurrency — cross-analysed against the doc 05 two-phase baseline. Prerequisites: fact-triple uniqueness + upsert, actionable MCP errors, forward-redirect tombstone. Rule 2 cadence anchoring for reconcile + temporal alignment. Branch `feat/parallel-ingestion`. ⚠️ **Doc-number 38 collides** with Graph-Anchored Fallback Retrieval above — the two docs were authored independently on `feat/cognitive-platform-v1` and `feat/parallel-ingestion`; filenames differ (`38-graph-anchored-fallback-retrieval.md` vs `38-parallel-ingestion.md`) and both branches' code comments reference "doc 38", so renumbering is deferred to a follow-up (see merge notes). |
