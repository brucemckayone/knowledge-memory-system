...
Create the Central Controller agent that orchestrates all Gardener operations. This is the coordination hub in the KARMA architecture — responsible for priority scheduling, resource management, and metrics recording.

> **Implementation note (2026-03-16):** MAB (Multi-Armed Bandit) scheduling was removed (migration 010 drops `mab_state`). The controller now uses simple priority + tier defaults. The controller also records per-job metrics directly to `gardener_metrics` (replacing the former evaluator agent). The `AgentContext` interface was enriched with `traceId`, `config`, `services` (ml + controller), and `signal` (AbortSignal with tier-based timeout). Gardener observability was added in migration 012 (`gardener_agent_stats` view).
...
### Role in 7-Agent Architecture
```
┌──────────────────────────────────────────────────────────────┐
│                  CENTRAL CONTROLLER                            │
│    ┌─────────────────────────────────────────────────────┐   │
│    │  • Priority + Tier Defaults (realtime/frequent/      │   │
│    │    periodic)                                          │   │
│    │  • Per-job Metrics Recording (gardener_metrics)      │   │
│    │  • Typed Error Handling (retry vs terminal)           │   │
│    │  • Checkpointing                                     │   │
│    │  • AbortSignal with tier-based timeout                │   │
│    └─────────────────────────────────────────────────────┘   │
│         │                                                     │
│         ▼                                                     │
│    [Dispatches jobs to 7 agents]                             │
└──────────────────────────────────────────────────────────────┘
```
