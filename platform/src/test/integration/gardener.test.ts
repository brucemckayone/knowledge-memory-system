 ...
 * `gardener_metrics`) that was prototyped but never migrated. The whole
 * suite is gated with a top-level-await table-existence check via
 * describe.skipIf so it skips cleanly on branches that don't ship those
 * tables. The previous beforeAll/skipCtx approach did not propagate
 * reliably to nested describes.
 */
const HAS_GARDENER_TABLES = (
  await testDb`
...
describe.skipIf(!HAS_GARDENER_TABLES)('Gardener Controller ↔ Agents Integration', () => {
