...
function showNodeDetail(d) {
  ...
  // Appended to the existing detail; populated asynchronously after innerHTML
  // is set so the panel renders immediately and the impact fetch streams in.
...
// ============================================
// Phase 4 — Impact subpanel (blast radius, doc 15)
// ============================================

/**
 * Fetch the blast-radius report for a node and render it into #impact-section.
 * Mounted by showNodeDetail when an entity or causalEvent is selected.
 *
 * `opts.hypothetical` — when 'expire', re-scores severity as if the node were
 * about to be expired. Makes ZERO database writes; severitySummary may move to
 * critical for sole-evidence citations.
 */
async function fetchImpact(nodeType, nodeId, opts) {
  const section = document.getElementById('impact-section');
  if (!section) return;

  const hypothetical = opts && opts.hypothetical ? opts.hypothetical : null;
  const queryString = hypothetical
    ? `?hypothetical=${encodeURIComponent(hypothetical)}&depth=3`
    : '?depth=3';

  try {
    const res = await fetch(`/api/impact/${encodeURIComponent(nodeType)}/${encodeURIComponent(nodeId)}${queryString}`);
    if (!res.ok) {
      const body = await res.json().catch(() => ({}));
      section.innerHTML = `<div class="impact-loading">Impact unavailable: ${esc(body.error || res.statusText)}</div>`;
      return;
    }
...
// ============================================
// Phase 6 — Pattern overlay (nmemo-d9v.15)
// ============================================
async function updatePatternsBadge() {
  try {
...
updatePatternsBadge();
setInterval(updatePatternsBadge, 5000);
